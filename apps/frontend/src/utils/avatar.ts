// RFC 1321 MD5 implementation in pure TypeScript for Gravatar avatar generation
...
/**
 * Returns a Gravatar avatar URL with retro/identicon fallback derived deterministically from email.
 * If no email is present, falls back to the username seed.
 */
export function getGravatarUrl(emailOrSeed?: string | null, size = 128): string {
  const seed = (emailOrSeed || "player").trim().toLowerCase();
  const hash = md5(seed);
